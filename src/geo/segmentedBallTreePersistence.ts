...
import type {
  ...
  SegmentedBallTreeSnapshotSegment,
} from './segmentedBallTreeGeoIndex'

export type SegmentedBallTreeManifest = {
...
function encodedSegmentSize(segment: SegmentedBallTreeSnapshotSegment): number {
  return (
...
function readString(view: DataView, bytes: Uint8Array, offset: number): {
  ...
  if (offset + length > bytes.byteLength) {
    throw new Error('Segmented ball-tree string exceeds file length.')
  }
  return {
...
export function encodeSegmentedBallTreeSnapshot(
  ...
  for (const segment of snapshot.segments) {
    ...
    for (const node of segment.nodes) {
      ...
      offset += 4
      view.setFloat64(offset, node.centerLat, true)
      offset += 8
      view.setFloat64(offset, node.centerLon, true)
      offset += 8
      view.setFloat64(offset, node.radiusMeters, true)
      offset += 8
...
export function decodeSegmentedBallTreeSnapshot(
  buffer: ArrayBuffer,
): SegmentedBallTreeSnapshot {
  const view = new DataView(buffer)
  const bytes = new Uint8Array(buffer)
  if (buffer.byteLength < HEADER_BYTES || readAscii(view, 0, 8) !== MAGIC) {
    throw new Error('Segmented ball-tree index data has an invalid header.')
  }
  ...
  if (version !== FORMAT_VERSION) {
    throw new Error('Segmented ball-tree index data version is unsupported.')
  }
  ...
  const segments: SegmentedBallTreeSnapshotSegment[] = []
  let pointCount = 0
  ...
  for (let segmentIndex = 0; segmentIndex < segmentCount; segmentIndex += 1) {
    ...
    offset += 4
    const nodes: SegmentedBallTreeSnapshotSegment['nodes'] = []
    ...
    for (let nodeIndex = 0; nodeIndex < nodeCount; nodeIndex += 1) {
      ...
      offset += 4
      const centerLat = view.getFloat64(offset, true)
      offset += 8
      const centerLon = view.getFloat64(offset, true)
      offset += 8
      const radiusMeters = view.getFloat64(offset, true)
      offset += 8
    ...
    if (points.length !== segmentPointCount) {
      throw new Error('Segmented ball-tree segment point count mismatch.')
    }
    segments.push({
      id: id.value,
      isDelta,
  ...
  if (offset !== buffer.byteLength) {
    throw new Error('Segmented ball-tree index data has trailing bytes.')
  }
...
export function createSegmentedBallTreeManifest(
  snapshot: SegmentedBallTreeSnapshot,
  catalogEpoch: number,
  dataChecksum: string,
...
export function validateSegmentedBallTreeManifest(
  manifest: SegmentedBallTreeManifest,
  catalogEpoch: number,
): void {
  if (
    manifest.engineId !== 'segmented-ball-tree' ||
    manifest.engineVersion !== 1 ||
    manifest.catalogEpoch !== catalogEpoch
  ) {
    throw new Error('Segmented ball-tree index manifest does not match catalog.')
  }
}
